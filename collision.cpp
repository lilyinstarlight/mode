...
SDL_Rect CollisionStrategy::intersection(const Drawable & obj1, const Drawable & obj2) const {
	unsigned int x1 = max(obj1.get_x(), obj2.get_x());
	unsigned int y1 = max(obj1.get_y(), obj2.get_y());
	unsigned int x2 = min(obj1.get_x() + obj1.get_width(), obj2.get_x() + obj2.get_width());
	unsigned int y2 = min(obj1.get_y() + obj1.get_height(), obj2.get_y() + obj2.get_height());
...
bool CircularCollisionStrategy::check(const Drawable & obj1, const Drawable & obj2) const {
	...
	float collision_distance = r1/2.0 + r2/2.0;

	Vector2f mid1 = {obj1.get_x() + obj1.get_width()/2.0, obj1.get_y() + obj1.get_height()/2.0};
	Vector2f mid2 = {obj2.get_x() + obj2.get_width()/2.0, obj2.get_y() + obj2.get_height()/2.0};
...
bool PixelCollisionStrategy::check(const Drawable & obj1, const Drawable & obj2) const {
	...
	const SDL_Surface * s1 = obj1.get_surface();
	const SDL_Surface * s2 = obj2.get_surface();
	...
	for (unsigned int col = area.x; col < area.x + area.w; ++col) {
		for (unsigned int row = area.y; row < area.y + area.h; ++row) {
